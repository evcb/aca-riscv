...
class Blink(frequency: Int, baudRate: Int) extends Module {
  val io = IO(new Bundle {
    val txd = Output(UInt(1.W))
  ...
  val msg = Mux(blkReg === 1.U, '1'.asUInt(8.W), '0'.asUInt(8.W))

  tx.io.channel.bits := msg
  tx.io.channel.valid := blkReg =/= RegNext(blkReg)
}


class Count(frequency: Int, baudRate: Int) extends Module {
  val io = IO(new Bundle {
    val txd = Output(UInt(1.W))
    val led = Output(UInt(1.W))
    val rxd = Input(UInt(1.W))
  ...
  val tx = Module(new BufferedTx(frequency, baudRate))
  val cnt :: res :: Nil = Enum(2)
  val stateReg = RegInit(cnt)
  val cntReg = RegInit(0.U(8.W))
  tx.io.channel.bits := cntReg
  ...
  when(stateReg === cnt) {
    when(tx.io.channel.ready) {
      cntReg := cntReg + 1.U
      tx.io.channel.bits := cntReg + '0'.asUInt(8.W)
...
class BlinkReceiver(frequency: Int, baudRate: Int) extends Module {
  val io = IO(new Bundle {
    val txd = Output(UInt(1.W))
    val led = Output(UInt(1.W))
    val rxd = Input(UInt(1.W))
  ...
  // Receives ASCII encoding
  // If 1, turn on light
  when(rx.io.channel.bits === 48.asUInt(8.W)) {
    io.led := false.B
  }.otherwise{
    io.led := true.B
  }
...
class RiscvMain(frequency: Int, baudRate: Int) extends Module {
  val io = IO(new Bundle {
    val rxd = Input(UInt(1.W))
    val led = Output(UInt(1.W))
    val txd = Output(UInt(1.W))
...
//  }
  ...
  val b = Module(new BlinkReceiver(frequency, baudRate))
  b.io.rxd := io.rxd
  io.led := b.io.led
  io.txd := b.io.txd
...
object RiscvMain extends App {
  chisel3.Driver.execute(Array("--target-dir", "generated"), () => new RiscvMain(50000000, 115200))
}