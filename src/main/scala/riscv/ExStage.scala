...
class ExStage extends Module {
  ...
  //Parse idExIn
  val idExD1 = io.idExIn(ID_EX_D1, ID_EX_D2 + 1).asSInt// read data 1
  val idExD2 = io.idExIn(ID_EX_D2, ID_EX_IMM + 1).asSInt //read data 2
  val idExImm = io.idExIn(ID_EX_IMM, ID_EX_F + 1).asSInt  //immediate
  ...
  val outputMux1 = Wire(SInt(SZ_32_BIT)) // internal mux1 output
  val outputMux2 = Wire(SInt(SZ_32_BIT)) // internal mux2 output
  val outputMux3 = Wire(SInt(SZ_32_BIT)) // internal mux3 output
